...
이 문서는 `From My Diary` MVP에서 사용할 Supabase 데이터베이스, 인증, Storage, 보안 정책의 기준을 정리한다. 구현 전에는 최신 Supabase 문서와 changelog를 확인하고, 구현 후에는 실제 SQL과 policy 동작을 검증한다.
...
- `title`과 `body`의 최대 길이는 구현 단계에서 UI 입력 제한과 함께 확정한다.
...
- 업로드 가능한 파일 타입과 최대 크기는 구현 단계에서 UI와 함께 확정한다.
...
- [ ] Supabase MCP 재인증을 완료한다.
- [ ] Supabase changelog에서 Auth, Storage, RLS 관련 breaking change를 확인한다.
- [ ] 최신 Supabase Auth 문서에서 GitHub OAuth 설정을 확인한다.
- [ ] 최신 Supabase Storage 문서에서 private bucket policy 작성 방식을 확인한다.
- [ ] 최신 Supabase RLS 문서에서 policy syntax를 확인한다.

## 구현 후 검증

- [ ] 로그인한 사용자가 본인 일기만 조회할 수 있다.
- [ ] 로그인한 사용자가 본인 일기만 생성할 수 있다.
- [ ] 로그인한 사용자가 본인 일기만 수정할 수 있다.
- [ ] 로그인한 사용자가 본인 일기만 삭제할 수 있다.
- [ ] 다른 사용자의 `diary_entries` row에 접근할 수 없다.
- [ ] 다른 사용자의 Storage object에 접근할 수 없다.
- [ ] `service_role` 키가 클라이언트 코드, `.env`, Vercel 환경변수에 존재하지 않는다.
