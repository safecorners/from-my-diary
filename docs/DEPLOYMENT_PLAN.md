...
이 문서는 `From My Diary`를 Vite + React + Tailwind 기반 정적 SPA로 Vercel에 배포하기 위한 다음 단계 기준을 정리한다. 현재 MVP 앱 구현과 로컬 Vite dev server 검증은 완료되었으며, 다음 단계는 Vercel 프로젝트 연결과 production URL 검증이다. Next.js, 서버 API, Edge Functions는 사용하지 않는다.
...
- Vercel 프로젝트 생성, GitHub 저장소 연결, 환경변수 등록은 다음 단계에서 Codex가 진행한다.
...
MVP 구현 완료 시점에 루트 `vercel.json`으로 생성했다.

## 배포 전 체크리스트

- [x] GitHub 저장소가 준비되어 있다.
- [x] MVP 앱 코드가 `safecorners/from-my-diary`의 `main`에 merge되어 있다.
- [x] SPA rewrite 설정이 루트 `vercel.json`에 추가되어 있다.
- [x] 로컬 의존성을 설치한다.
- [x] `npm run build`가 성공한다.
- [x] 로컬 MVP 성공 기준을 검증한다.
